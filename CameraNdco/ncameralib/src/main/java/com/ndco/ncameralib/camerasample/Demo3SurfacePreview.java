 ...
 */
public class Demo3SurfacePreview extends SurfaceView implements SurfaceHolder.Callback {
    public Demo1CameraActivity myActivity;
    private static final String TAG = Demo3SurfacePreview.class.getSimpleName();
    private SurfaceHolder mHolder;
    private Camera mCamera;
    private Camera.Parameters mParameters;

    public Demo3SurfacePreview(Context context) {
        super(context);
...
//            mCamera.startPreview();
    ...
    //resetCamera
    private void resetCameraContinuousFocus() {
//        mCamera.stopPreview();
...
//                mCamera.autoFocus(foucsCallback);
    ...
    Camera.AutoFocusCallback foucsCallback = new Camera.AutoFocusCallback() {
        ...
        public void onAutoFocus(boolean success, Camera camera) {
            ...
            if(success){
                Log.d(TAG, "onAutoFocus() is success3");
                mCamera.takePicture(null, null, tackpicturecallback);

            }
            else{
                Log.d(TAG, "onAutoFocus() is failures3");
...
//                mCamera.stopPreview();
    ...
    void TackPictureAtAutoFocusMode(){
        if (mCamera != null) {
            mCamera.cancelAutoFocus();
            ...
            area.add(new Camera.Area(new Rect(-100,-100,100,100), 1));
